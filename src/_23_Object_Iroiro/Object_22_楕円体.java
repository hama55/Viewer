	...
	*/
	...
	{		
		try{
			...
			//	これを核として発展させていく。
			/////////////////////////////////////////////////////////
			Value_DoubleComplex[][] points_3 = create_最初の6角形を作る_1();
//			ArrayList<Test6角形> arry_test_point = create_最初の6角形を作る_2(points_around_shusoku_point);
			
//			/////////////////////////////////////////////
//			//	どんどん発展させていく
//			/////////////////////////////////////////////
//			for(int ic=0; ic<arry_test_point.size(); ic++){
//				arry_test_point.get(ic).六角形の足りない点を作成();
//			}
//			
//			for(int ic=0; ic<arry_test_point.size(); ic++){
//				/////////////////////////////////////
//				//重複要素を結合。
//				/////////////////////////////////////
//				arry_test_point.get(ic).隣り合う六角形と重複する点を結合();
//				
//
//				//収束させる。●
//				
...
//			for(Test6角形 buf_test : arry_test_point){
//				if(arry_set_point.contains(buf_test.get_point()) == false){
//					arry_set_point.add(buf_test.get_point());
//				}
//				
//				for(int ic=0; ic<6; ic++){
//					Object_03_Point buf_point = buf_test.get_test_point(ic+1).get_point();
//					if(buf_point == null) continue;
//					if(arry_set_point.contains(buf_point) == true) continue;
//					arry_set_point.add(buf_point);
//				}
//			}
...
//					"",
//					new Value_DoubleComplex(points_around_shusoku_point[ic][0]),
//					new Value_DoubleComplex(points_around_shusoku_point[ic][1]),
//					new Value_DoubleComplex(points_around_shusoku_point[ic][2])
//				);
//				group.getChildren().add(buf_point.ask_JavafxNode());
//			}
	...
	private int create_最初の6角形を作る_中心点_0点(
...
//		double[][] vec_法線 = new double[3][2];
		...
		System.out.printf("法線ベクトル %s", util.newline());for(int ic=0; ic<3; ic++){System.out.printf("%f, %f %s",vec_法線[ic][0], vec_法線[ic][1], util.newline());}
		
		//収束値でPointを作る
//		Object_03_Point point = new Object_03_Point("", point_zero);
		
		return n_count;
	}
	private int create_最初の6角形を作る_1点目(
...
//		/////法線ベクトルとX軸ベクトルを直交化
//		//X軸ベクトル
//		double[][] vector_target = new double[][]{
//			{1,0},
//			{0,0},
//			{0,0}
//		};
...
//		//グラムシュミットの直交化
//		Mathmatic.calc_Gram_Schmidt_vector_complex(vector_target, vec_法線, result);
//		System.out.printf("グラムシュミットの直交化 X軸 %s", util.newline());for(int ic=0; ic<3; ic++){System.out.printf("%f, %f %s",result[0][0], result[0][1], util.newline());}
//
//		//削ったターゲットの長さが小さいなら、X軸ではなくY軸で行う。誤差抑えるため。
//		if(Mathmatic.calc_length_vector_complex(3, result) <= 0.5){
//			//Y軸ベクトル
//			vector_target = new double[][]{
//				{0,0},
//				{1,0},
//				{0,0}
//			};
//			//グラムシュミットの直交化
//			Mathmatic.calc_Gram_Schmidt_vector_complex(vector_target, vec_法線, result);	
//			System.out.printf("グラムシュミットの直交化 Y軸 %s", util.newline());for(int ic=0; ic<3; ic++){System.out.printf("%f, %f %s",result[0][0], result[0][1], util.newline());}
//		}
		/////削りだしたベクトルを伸ばす。そして収束点位置に移動。それが6角形の1点目になる。
		double[] pitch = new double[]{this.m_pitch, 0}; 
//		//ユニット化
//		Mathmatic.calc_vector_unit_complex(result, 3, result);

		double[][] result = new double[3][2];
		Mathmatic.calc_直交化ベクトルの簡単出力_complex(vec_normal_tool, vec_直交);
		System.out.printf("直交化ベクトルの簡単出力 %s", util.newline());for(int ic=0; ic<3; ic++){System.out.printf("%f, %f %s",result[ic][0], result[ic][1], util.newline());}
		...
		System.out.printf("1点目 %s", util.newline());for(int ic=0; ic<3; ic++){System.out.printf("%f, %f %s",point1[ic][0], point1[ic][1], util.newline());}

//		//求めた値を取得
//		for(int ic=0; ic>3; ic++){
//			point_1st[ic] = new Value_DoubleComplex(point1[ic][0], point1[ic][1]);
//		}
		...
		System.out.printf("反復回数(1点目) :%d%s", buf_n_count, util.newline());
		
		return 0;
	}
	private void create_最初の6角形を作る_2点目(
		Value_DoubleComplex[]	point_zero,	//I		
		double[][]				vec_法線,	//I		
...
//		//求めた値を取得
//		for(int ic=0; ic>3; ic++){
//			point_2nd[ic] = new Value_DoubleComplex(point2[ic][0], point2[ic][1]);
//		}
		...
		System.out.printf("反復回数(2点目) :%d%s", buf_n_count, util.newline());
	 ...
	 */
	//************************************************************************//
	private Value_DoubleComplex[][] create_最初の6角形を作る_1()
	{
		///////////////////////////////////////
		//	中心点（0点）作成
		///////////////////////////////////////
		...
		double[][] vec_法線 = new double[3][2];
		int n_count = create_最初の6角形を作る_中心点_0点(
			point_zero,	//O		
			vec_法線		//O		
		);
...
//		double[][][] points_around_shusoku_point_and_center = new double[7][3][2];
//		points_around_shusoku_point_and_center[1] = point1;
//		//1点目を回転させていき、6点すべて出す
//		for(int ic=2; ic<=6; ic++)
//		{
//			Mathmatic.calc_rotate_around_axis2_2_complex(shusoku_point,					//I		
//				vec_法線,				//I		
//				point1,							//I
//				60*(ic-1),							//I
//				points_around_shusoku_point_and_center[ic]	//O		
//			);
//			//テスト
//			System.out.printf("%d点目 %s", ic, util.newline());
//			System.out.printf("%f, %f %s",points_around_shusoku_point_and_center[ic][0][0], points_around_shusoku_point_and_center[ic][0][1], util.newline());
//			System.out.printf("%f, %f %s",points_around_shusoku_point_and_center[ic][1][0], points_around_shusoku_point_and_center[ic][1][1], util.newline());
//			System.out.printf("%f, %f %s",points_around_shusoku_point_and_center[ic][2][0], points_around_shusoku_point_and_center[ic][2][1], util.newline());
//		}
//		points_around_shusoku_point_and_center[0][0][0] = candidate_values_x.get_re();
//		points_around_shusoku_point_and_center[0][0][1] = candidate_values_x.get_im();
//		points_around_shusoku_point_and_center[0][1][0] = candidate_values_y.get_re();
//		points_around_shusoku_point_and_center[0][1][1] = candidate_values_y.get_im();
//		points_around_shusoku_point_and_center[0][2][0] = candidate_values_z.get_re();
//		points_around_shusoku_point_and_center[0][2][1] = candidate_values_z.get_im();
//
//		for(int ic=1; ic<=6; ic++){
//			//6点を収束計算
//			ArrayList<Value_DoubleComplex> buf_arry_candidate_values = new ArrayList<>(3);
//			buf_arry_candidate_values.add(new Value_DoubleComplex(points_around_shusoku_point_and_center[ic][0]));
//			buf_arry_candidate_values.add(new Value_DoubleComplex(points_around_shusoku_point_and_center[ic][1]));
//			buf_arry_candidate_values.add(new Value_DoubleComplex(points_around_shusoku_point_and_center[ic][2]));
//			//収束計算
//			int buf_n_count = Mathmatic_By_Value1.calc_楕円体上の収束値をがむしゃらに探す(
//				buf_arry_candidate_values
//			);
//			Value_DoubleComplex buf_candidate_values_x = buf_arry_candidate_values.get(0);		//IO	X値
//			Value_DoubleComplex buf_candidate_values_y = buf_arry_candidate_values.get(1);		//IO	Y値
//			Value_DoubleComplex buf_candidate_values_z = buf_arry_candidate_values.get(2);		//IO	Z値
//			System.out.printf("%d点目 反復回数%d %s", ic, buf_n_count, util.newline());
//			System.out.printf("%f, %f %s",buf_candidate_values_x.get_re(), buf_candidate_values_x.get_im(), util.newline());
//			System.out.printf("%f, %f %s",buf_candidate_values_y.get_re(), buf_candidate_values_y.get_im(), util.newline());
//			System.out.printf("%f, %f %s",buf_candidate_values_z.get_re(), buf_candidate_values_z.get_im(), util.newline());
//		}
